...
export default function EditReservationPage() {
  ...
  const fetchReservation = async () => {
    try {
      ...
      const reservation = await reservationApi.getById(params.id as string)
      ...
      setFormData({
        ...
        room: typeof reservation.room === 'object' ? reservation.room._id : reservation.room,
      ...
      // Set selected room if room data is available
      if (typeof reservation.room === 'object') {
  ...
  const fetchRooms = async () => {
    try {
      const response = await roomApi.getAll({ limit: 1000 })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!availability.available && availability.conflictingReservation !== params.id) {
        setErrors({ room: 'Room is not available for the selected dates' })
        return
      }

      // Update reservation
      await reservationApi.update(params.id as string, formData)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <Link href={`/reservations/${params.id}`} className="text-gray-600 hover:text-gray-900 mr-4">
